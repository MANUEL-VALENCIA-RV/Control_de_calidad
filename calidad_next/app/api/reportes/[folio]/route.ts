...
export async function PATCH(request: Request, { params }: { params: Promise<{ folio: string }> }) {
  try {
    ...
    const res = await fetch(nestUrl(`/reportes/${encodeURIComponent(folio)}`), { method: "PATCH", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", Accept: "application/json" }, body, cache: "no-store" });
