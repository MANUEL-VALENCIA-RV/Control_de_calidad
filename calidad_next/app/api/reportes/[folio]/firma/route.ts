...
export async function POST(request: Request, { params }: { params: Promise<{ folio: string }> }) {
  try {
    ...
    const res = await fetch(nestUrl(`/reportes/${encodeURIComponent(folio)}/firma`), { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: form, cache: "no-store" });
    return proxyJson(res);
  } catch (e) { return NextResponse.json({ error: e instanceof Error ? e.message : "Error de conexión" }, { status: 502 }); }
}

export async function DELETE(_request: Request, { params }: { params: Promise<{ folio: string }> }) {
  try {
    const token = await sessionToken(); if (!token) return NextResponse.json({ error: "No autorizado" }, { status: 401 });
    const { folio } = await params;
    const res = await fetch(nestUrl(`/reportes/${encodeURIComponent(folio)}/firma`), { method: "DELETE", headers: { Authorization: `Bearer ${token}` }, cache: "no-store" });
    return proxyJson(res);
  } catch (e) { return NextResponse.json({ error: e instanceof Error ? e.message : "Error de conexión" }, { status: 502 }); }
}
